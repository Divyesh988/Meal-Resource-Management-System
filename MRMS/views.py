...
from FCRs.models import area_master, FoodCollectionRequests, feedbacks_master_byUsers, feedbacks_master_byFC
import requests
...
def User_view_fcrs(request,email=None):
    all_reqs = FoodCollectionRequests.objects.filter(complainant_email=email) 
    user = CustomUser.objects.get(email=email)
    user_profile = UserProfile.objects.get(user=user)
    areas =area_master.objects.all()
    context = {
...
def FC_view_fcrs(request,email=None):
    ...
    all_reqs = FoodCollectionRequests.objects.filter(area=gc_area) 
    areas =area_master.objects.all()
    context = {
        'user' : user,
        'user_profile': user_profile,
...
def User_view_fcr(request,email=None,id=None):
    all_reqs = None
    all_reqs = FoodCollectionRequests.objects.filter(complaint_id=id) 
    all_req = all_reqs.first()
    user = CustomUser.objects.get(email=email)
    user_profile = UserProfile.objects.get(user=user)
    areas =area_master.objects.all()
    context = {
...
def FC_view_fcr(request,email=None,id=None):
    if request.method == 'POST':
        ...
        GC_id = str(GC_id)
        all_reqs = FoodCollectionRequests.objects.get(complaint_id=complaint_id)
        all_reqs.status = 'Completed'
        all_reqs.changed_by = GC_id
        all_reqs.save()
        ...
        all_reqs = None
        all_reqs = FoodCollectionRequests.objects.filter(complaint_id=id) 
        complaint_details = all_reqs.first()
        user = CustomUser.objects.get(email=email)
        user_profile = FoodCollectorProfile.objects.get(user=user)
        areas =area_master.objects.all()
        context = {
